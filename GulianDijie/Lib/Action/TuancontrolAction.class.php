...
//	}
    ...
    public function baozhangdan() {
		...
		if($_GET['showpage'] == '计调'){
			...
			if($_GET['doprint']) $itemAll = $DJbaozhangitem->where("`baozhangID` = '$baozhang[baozhangID]' and (`check_status` = '经理确认' or `check_status` = '审核通过') and (`is_print` is null or `is_print` = '')")->findall();
		}
		else{
			$itemAll = $DJbaozhangitem->where("`baozhangID` = '$baozhang[baozhangID]' and `check_status` = '审核通过'")->findall();
			if($_GET['doprint']) $itemAll = $DJbaozhangitem->where("`baozhangID` = '$baozhang[baozhangID]' and `check_status` = '审核通过' and (`is_print` is null or `is_print` = '')")->findall();
		}
