...
//		$rurl = SITE_ADMIN."Kongguan/sankekongguan";
    ...
    public function dopostpiliangjiezhi() {
		...
		foreach($postdata['itemlist'] as $zituanID)
		{
			...
			$zituan1 = $Glzituan->where("`zituanID` = '$zituanID'")->find();
			F_xianlu_status_set($zituan1['xianluID']);
			//记录
...
//		$rurl = SITE_ADMIN."Kongguan/sankekongguan";
    ...
    public function dopostpilianghuifu() {
		...
		foreach($postdata['itemlist'] as $zituanID)
		{
			...
			$zituanl = $Glzituan->where("`zituanID` = '$zituanID'")->find();
			F_xianlu_status_set_2($zituanl['xianluID']);
			//记录
...
//		$this->assign('zhichuheji',$zhichuheji);
	...
	public function exports_word() {
		...
		$tuanyuan = $Gltuanyuan->where("`zituanID` = '$zituanID' AND `tuanyuanID` != '$leader[tuanyuanID]'")->order('dingdanID ASC')->findall();
		...
		while($page < $page_num){
			$start_pos = $page*20;
			$tuanyuan_part[$page] = $Gltuanyuan->where("`zituanID` = '$zituanID' AND `tuanyuanID` != '$leader[tuanyuanID]'")->order('dingdanID ASC')->limit($start_pos.',20')->select();
			$page++;
		}
