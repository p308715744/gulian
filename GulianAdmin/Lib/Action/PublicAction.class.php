...
//		exit;
	...
	//重新修正以前的分类kind字段
	public function modify_kind() {
		exit;
		$gl_xianlu = D("Glxianlu");
		$gl_zituan = D("Glzituan");
		
		$condition['xianlutype'] = "散客产品";
		$condition['guojing'] = "境外";
		$condition['kind'] = '出境游';
		
		$xianluAll = $gl_xianlu->where($condition)->findall();
		$zituanAll = $gl_zituan->where($condition)->findall();
		
		foreach($xianluAll as $xianlu){
			if($xianlu['departmentName'] == '日本部-出境'){
				$xianlu['kind'] = "日本";	
			}elseif('出境部-韩国' == $xianlu['departmentName']){
				$xianlu['kind'] = "韩国";
			}elseif('出境部-台湾' == $xianlu['departmentName']){
				$xianlu['kind'] = "台湾";
			}elseif('出境部-港澳东南亚' == $xianlu['departmentName']){
				if(strstr($xianlu['mingcheng'] ,'港') || strstr($xianlu['mingcheng'] ,'澳'))	$xianlu['kind'] = "港澳";
				else $xianlu['kind'] = "东南亚";
			}elseif('出境部-欧美岛' == $xianlu['departmentName']){
				if($xianlu['kind'] == '欧美澳非') continue;
				elseif($xianlu['kind'] == '海岛游') continue;
				else $xianlu['kind'] = "欧美游";
			}
			dump($xianlu['xianluID']);
			$gl_xianlu->save($xianlu);
		}
		
		foreach($zituanAll as $zituan){
			if($zituan['departmentName'] == '日本部-出境'){
				$zituan['kind'] = "日本";	
			}elseif('出境部-韩国' == $zituan['departmentName']){
				$zituan['kind'] = "韩国";
			}elseif('出境部-台湾' == $zituan['departmentName']){
				$zituan['kind'] = "台湾";
			}elseif('出境部-港澳东南亚' == $zituan['departmentName']){
				if(strstr($zituan['mingcheng'] ,'港') || strstr($zituan['mingcheng'] ,'澳'))	$zituan['kind'] = "港澳";
				else $zituan['kind'] = "东南亚";
			}elseif('出境部-欧美岛' == $zituan['departmentName']){
				if($zituan['kind'] == '欧美澳非') continue;
				elseif($zituan['kind'] == '海岛游') continue;
				else $zituan['kind'] = "欧美游";
			}
			dump($zituan['zituanID']);
			$gl_zituan->save($zituan);
		}
	...
	public function tuanyuanlist()
	{
		...
		$condition['telnum'] = array("egt","0");
		
		import("@.ORG.Page");
		C('PAGE_NUMBERS',50);
		$count = $gltuanyuan->where($condition)->count();
		$p = new Page ( $count, 50 ); 
		$datalist=$gltuanyuan->limit($p->firstRow.','.$p->listRows)->where($condition)->order('dingdanID desc')->findAll(); 
		$p->setConfig('header','篇记录');
        $p->setConfig('prev',"上一页");
        $p->setConfig('next','下一页');
        $p->setConfig('first','首页');
	...
	public function export_tuanyuan()
	{
		header( "Content-type:   application/octet-stream "); 
		header( "Accept-Ranges:   bytes "); 
		header( "Content-Disposition:   attachment;   filename=名单.txt "); 
		header( "Expires:   0 "); 
		header( "Cache-Control:   must-revalidate,   post-check=0,   pre-check=0 "); 
		header( "Pragma:   public "); 
		
		$sql = "SELECT DISTINCT telnum From gltuanyuan Where telnum > 0";
		$rs = M()->query($sql);
		
		foreach($rs as $key => $tuanyuan){
			$tuanyuan['telnum'] = trim($tuanyuan['telnum']);
			$tuanyuan['telnum'] = str_replace(" ","",$tuanyuan['telnum']);
			if(preg_match("/^(13|15|18)\d{9}$/", $tuanyuan['telnum'])){
				$tels[$key]['telnum']  = $tuanyuan['telnum'];
			}
		}
    ...
    public function yuangongtongxunlu() {
		...
		$wheres['beizhu'] = array('between','1,1000');
		
		$kehuAll = $Glkehu->where($wheres)->order("beizhu asc")->findall();
