...
/*			if ($_REQUEST['start_date'] && $_REQUEST['end_date']){
			...
			$i = 0;
			$dingdanPage = $caiwu_dingdan_tongji->where($condition)->order("zituanID DESC")->limit($p->firstRow.','.$p->listRows)->findall();
...
/*			import("@.ORG.Page");
	...
	public function yuangongticheng() {
		...
		if ($types == 'Line'){
			$navlist = "账务管理 > 统计管理 > 员工提成";
			$condition['xianlutype'] = '散客产品';
			$condition_all['xianlutype'] = '散客产品';
		}
		if ($types == 'department'){
			$navlist = "账务管理 > 统计管理 > 部门提成";
			$condition['xianlutype'] = '散客产品';
			$condition_all['xianlutype'] = '散客产品';
		}
		...
		if ($types == 'Line' || $types == 'department'){
			...
			$_REQUEST['department'] ? $conditions['departmentName'] = $_REQUEST['departmentName']:'';
			$_REQUEST['department'] ? $where['departmentName'] = $_REQUEST['departmentName']:'';
			...
			$_REQUEST['department'] ? $wheres .= " AND `departmentName` = '".$_REQUEST['department']."'":'';
			$_REQUEST['user_name'] ? $wheres .= " AND `user_name` = '".$_REQUEST['user_name']."'":'';
			$_REQUEST['companyname'] ? $wheres .= " AND `companyname` = '".$_REQUEST['companyname']."'":'';
			$_REQUEST['guojing'] ? $wheres .= " AND `guojing` = '".$_REQUEST['guojing']."'":'';
			$_REQUEST['keyword'] ? $wheres .= " AND `mingcheng` LIKE '%".$_REQUEST['keyword']."%'":'';
			
			
			//联合子团和地接视图
			$sql = "SELECT * from (SELECT z.zituanID,z.caiwuren,z.caiwu_time,z.xianlutype,z.tuanhao,z.mingcheng,z.chutuanriqi,z.departmentID,z.departmentName,z.user_name,z.renshu,z.kind FROM zituan_baozhang z union all SELECT d.djtuanID,d.financeperson,d.caiwu_time,d.xianlutype,d.tuannumber,d.tuantitle,d.startdate,d.departmentID,d.departmentName,d.adduser,d.renshu,d.kind FROM djtuan_baozhang d) AS tmp ".$wheres;
			$rs = M()->query($sql);
			...
			if($types == 'department'){
				...
				{
					if($value['xianlutype'] == '地接'){
						$res[$value['departmentID']][$value['zituanID']] = get_informations('zituan', $value, $value['zituanID'], '1');
					}else{
						$res[$value['departmentID']][$value['zituanID']] = get_informations('zituan', $value, $value['zituanID'], 0);
					}
				}
			}
			else{
				...
				foreach($rs as $key=>$value)
				{
					...
					if($value['xianlutype'] == '地接'){
						$res[$value['user_name']][$value['zituanID']] = get_informations('user', $value, $value['zituanID'], '1');
					}else{
						$res[$value['user_name']][$value['zituanID']] = get_informations('user', $value, $value['zituanID'], 0);
					}
				}
				...
				$condition['dingdanID'] = array('neq','null');
				$condition['check_status'] = '审核通过';
				//$condition['d_status'] = '已确认';
				
				$_REQUEST['user_name'] ? $condition['owner'] = $_REQUEST['user_name']:'';
				$_REQUEST['department'] ? $condition['department'] = $_REQUEST['department']:'';
				$_REQUEST['keyword'] ? $condition['mingcheng'] = array('LIKE','%'.$_REQUEST['keyword'].'%'):'';
				...
				$kehu_lock = $glkehu->where("`islock` = '已锁定'")->findall();
				foreach($kehu_lock as $kehu){
					unset($res[$kehu['user_name']]);
				}
			}
		...
		$this->assign('tuan',$tuan);
		if($_GET['isprint']) {
			if($types == 'department') $this->display('ticheng_print_department');
			else	$this->display('ticheng_print');
		}
		else {
			if($types == 'department') $this->display('departmentticheng');
			else $this->display();
		}
	}
