...
class QianzhengAction extends CommonAction{
    ...
    public function qianzhenginfo() {
		...
		else if($type == 'djtuan') $condition['djtuanID'] = array('neq','0');
    ...
    public function addnew() {
		...
		if($_GET['doprint']){
			...
			if($_GET['printtype'] == '计调')
			{
				...
				elseif($qianzheng['kind'] == '补账') $this->display("print_buzhang_2");
			...
			else
			{
				...
				elseif($qianzheng['kind'] == '补账') $this->display("print_buzhang");
			...
			elseif($kind == '补账')	$this->display("addnew_buzhang");
        	else	$this->display();
		}
    }
...
/*				if($qianzheng['manager'])
		...
		else
		{
			...
			$reurl =SITE_ADMIN.'Qianzheng/qianzhenginfo/zituanID/'.$postdata['zituanID'].'/djtuanID/'.$postdata['djtuanID'].'/kind/'.$postdata['kind'];
		}
