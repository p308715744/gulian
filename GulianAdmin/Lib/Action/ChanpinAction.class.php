    ...
    public function deletezituan() {
		return false;
			...
			foreach($zituanlist as $zituanID)
			{
				...
				$zituan = $Glzituan->where($condition)->field('zituanID,chutuanriqi,zhuangtai,tuanhao')->find();
				if($zituan)
				{
					if($date)
				...
				if($zituan['zhuangtai'] == '准备')
				{
					...
					if($bzd || $dd)
						doalert('团号：'.$zituan['tuanhao']."，已被锁定不能删除",$rurl);
					else
			...
			//无判断，不可以删除  By heavenK 2012-07-03
			//删除子团
/*			foreach($zituanlist as $zituanID)
			{
				//检查状态
