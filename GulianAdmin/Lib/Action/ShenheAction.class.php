...
//					}
			...
			if($_GET['dotype'] == '财务总监通过' || $_GET['dotype'] == '财务总监不通过'){
					...
					if($djtuanID)
					{
							if(!$baozhangdan['departmentperson'] || !$baozhangdan['financeperson'])
									doalert("失败！财务未审核通过！",'');
							if($baozhangdan['manager'])
									doalert("失败！报账单正在上级审核中！",'');
					}
...
//end
    ...
    public function dingdanshenhe() {
		...
		if($dingdan)
		{
			if(!checkByAdminlevel('计调经理,网管,总经理,财务操作员,财务总监',$this))
			{
...
//						doalert("你无权审核别人子团内的订单",'');
			...
			if($_GET['roletype'] == '财务操作员'){
				...
				if($_GET['status'] == '不合格')	{
						$dingdan['status'] = '不合格';
						A("Message")->savemessage($dingdanID,'订单','审核记录',$_GET['roletype'].' 订单不合格！','计调操作员',$menshi_url);
				}
			}else{	
			...
			doalert($_GET['check_status'].'操作成功','');
		}
			else
			doalert("错误",'');
